...
TopMenu.prototype = {
  ...
  _initPlugin: function(plugin) {
    if (plugin.isInit) {
      ...
      plugin.isInit = true;
      return plugin.init(this) || plugin;
    }
  },
  _render : function() {
  ...
  _init : function() {
    ...
    // 设置菜单的宽度
    if (!p.isFit && p.width) {
      g.setMenuWidth(p.width);
    }

    // FIXME: 设置菜单内容宽度的功能需要重新审查
  ...
  /**
   * resize事件处理
   * FIXME: 菜单resize事件的处理方法可能会产生冲突，和其他元素的resize方法相互覆盖
   */
  _resize : function() {
    var g = this, p = this.options, dom = this.dom, elem = this.element;
  ...
  /**
   * 选中菜单项
   * @params index {number} 菜单项索引值
   * */
  selectMenuItem: function(index) {
    var g = this, p = this.options, dom = this.dom, elem = this.element;

    var $menuItems = g.wrap.find('.topMenu-item a');
    var $selectedItem = $menuItems.eq(index);

    $menuItems.removeClass('active');
    $selectedItem.addClass('active');

    if (p.childMenuDom) {
      $.extend( p.childMenuOptions, { data : p.data[index].menu } );
  ...
  setChildMenu : function( childMenuDom, childOptions ) {
    ...
    // FIXME: 子菜单重新加载的功能没有考虑重复加载的问题，可能需要优化
    //清除原有子菜单
    if ( g.childMenu ) {
